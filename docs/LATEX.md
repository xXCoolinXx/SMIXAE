...
## Static Browser Viewer (Netlify / Vercel Deployment)

A **read-only** version of the interactive viewer can be deployed as a static site — no Python server required. This allows others to browse and view experts without running the save server.

### How it works

The static viewer uses the same JS/CSS from `src/latex/viewer/` but fetches data from pre-generated flat files instead of the Python server API. Mode is detected automatically at runtime:

```js
const _SERVER = window.location.hostname === '127.0.0.1' && window.location.port === '7788';
```

When `true` → uses server API endpoints. When `false` → fetches from static paths.

### File layout (generated by `scripts/build_static.py`)

```
browser/
├── tasks.json                  ← task index (fetched by index.html)
├── colorscales/
│   └── {name}_{n}_{skip}.json  ← discrete + continuous colorscale samples
└── results/
    └── {rel}/experts/E*.bin    ← binary tensor blobs
```

Existing committed files used directly:
- `results/*/index.json` — task metadata
- `results/*/experts/E*.json` — per-expert metadata
...
Run locally before deploying, or set it as the Netlify build command.

### Deployment

1. Link the git repo to Netlify (or Vercel)
2. Set publish directory to repo root (`.`)
3. Build command: `uv run python scripts/build_static.py`
4. Netlify redirects `src/latex/viewer/*` → `src/latex/viewer/index.html` automatically, but a `_redirects` file may be needed:

```
/src/latex/viewer/*  /src/latex/viewer/index.html  200
```

### Landing page

`index.html` at repo root serves as the task browser. It fetches `browser/tasks.json`, groups tasks by experiment, and links each to `src/latex/viewer/index.html?task=<rel>`.

### Verify

```bash
# Serve locally (port != 7788 → static mode)
python -m http.server 8080

# Open http://localhost:8080/
# - Task browser shows all experiments/tasks
# - Click a task → viewer opens, 3D scatter renders
# - Save buttons are hidden (read-only)
```

The save server at port 7788 continues to work unchanged (`smixae latex save-server`).
